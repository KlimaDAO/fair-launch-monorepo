...
contract DeployBase is Script {
    ...
    function deployContracts(address owner) internal {
        ...
        bytes memory stakingInitData = abi.encodeWithSelector(KlimaFairLaunchStaking.initialize.selector, owner);

        // Deploy proxy
        console.log("Deploying KlimaFairLaunchStaking proxy...");
        address stakingProxyAddress = address(new ERC1967Proxy(stakingImplementation, stakingInitData));
        ...
        // Prepare initialization data
        bytes memory burnVaultInitData =
            abi.encodeWithSelector(KlimaFairLaunchBurnVault.initialize.selector, owner, BASE_INTERCHAIN_SERVICE);

        // Deploy proxy
        console.log("Deploying KlimaFairLaunchBurnVault proxy...");
        address burnVaultProxyAddress = address(new ERC1967Proxy(burnVaultImplementation, burnVaultInitData));
