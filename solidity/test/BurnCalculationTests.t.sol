...
contract BurnCalculationTests is Test {
    ...
    function test_RoundingInBurnCalculation() public {
        ...
        console.log("burnAmount immediately after stake", burnAmount);
        assertEq(burnAmount, 25e9);

        vm.warp(startTime + 1);
        burnAmount = staking.calculateBurn(stakeAmount, startTime);
        console.log("burnAmount after 1 second", burnAmount);
        assertGt(burnAmount, 25e9);

        vm.warp(startTime + 116 hours);
        burnAmount = staking.calculateBurn(stakeAmount, startTime);
        console.log("burnAmount after 116 hours", burnAmount);
        assertGt(burnAmount, 25e9);

        vm.warp(startTime + (365 days / 2));
        burnAmount = staking.calculateBurn(stakeAmount, startTime);
        console.log("burnAmount after 182.5 days", burnAmount);
        assertEq(burnAmount, (25e9 + (75e9 / 2)));

        vm.warp(startTime + 366 days);
        burnAmount = staking.calculateBurn(stakeAmount, startTime);
        console.log("burnAmount after 366 days", burnAmount);
        assertEq(burnAmount, 100e9);
